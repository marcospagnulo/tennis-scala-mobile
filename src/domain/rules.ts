...
function pairIncludesPlayers(match: Match, playerId: string, opponentId: string) {
  return (
...
export function getRankedPlayers(players: Player[], matches: Match[]): RankedPlayer[] {
  return players.map((player) => {
    const currentPeriodMatches = matches.filter(
      (match) => match.homePlayerId === player.id || match.awayPlayerId === player.id,
    )
...
export function buildGroups(players: RankedPlayer[], mode: RankingMode, targetGroupSize: number) {
...
function isBoundaryPlayer(playerId: string, group: GroupedRanking, boundarySize: number, edge: 'top' | 'bottom') {
  const slice =
    edge === 'top' ? group.players.slice(0, boundarySize) : group.players.slice(-boundarySize)
...
export function evaluateChallenge(
  ...
  if (!canChallengeAcrossGroups(challenge.challengerId, challenge.opponentId, realGroups, rules.adjacentBoundarySize)) {
    reasons.push('La sfida non rispetta i limiti tra stesso gruppo e giocatori di confine tra gruppi adiacenti.')
  }

  const challengerMatches = matches.filter(
    (match) => match.homePlayerId === challenger.id || match.awayPlayerId === challenger.id,
  )
  const opponentMatches = matches.filter(
    (match) => match.homePlayerId === opponent.id || match.awayPlayerId === opponent.id,
  )
  const pairMatches = matches.filter((match) => pairIncludesPlayers(match, challenger.id, opponent.id))
  ...
  if (pairMatches.length > 0) {
    ...
    const currentOpponents = new Set(
      challengerMatches.map((match) =>
        match.homePlayerId === challenger.id ? match.awayPlayerId : match.homePlayerId,
      ),
    )
    ...
    if (!hasPlayedEntireGroup) {
      reasons.push('La coppia ha gia giocato nello stesso periodo e non risulta completato il giro del gruppo dello sfidante.')
    }
  }

  if (challenge.type === 'irrevocable') {
    if (rules.maxMatchesPerPeriod !== undefined && opponentMatches.length >= rules.maxMatchesPerPeriod) {
      reasons.push("Lo sfidato ha gia raggiunto il numero massimo di incontri previsto per il periodo.")
    }

    if (rules.maxMatchesPerPeriod === undefined) {
      warnings.push('Il regolamento estratto non esplicita il numero massimo di incontri per periodo: parametro da configurare.')
    }
    ...
    if (hasBookedMatch) {
      reasons.push('Lo sfidato ha gia un incontro prenotato nella stessa data.')
    }
    ...
    if (
      opponentLatestMatch &&
      differenceInDays(challenge.scheduledAt, opponentLatestMatch) < rules.minimumRestDaysForIrrevocable
    ) {
      reasons.push('Tra due partite dello sfidato devono trascorrere almeno 2 giorni.')
    }
  }
...
export function isPlayerEligibleForFinal(player: RankedPlayer, rules: RuleConfig) {
  return player.status === 'active' && player.totalMatches >= rules.minimumMatchesForFinalStage
}
